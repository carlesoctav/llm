...
def _validate_inputs(
    ...
    if b_block_size is None or h_block_size is None or v_block_size is None:
        raise PallasUnsupportedError(f"Pallas requires non-None block sizes, got {block_sizes}.")
...
def _infer_core_grid(b_dim: int, block_sizes: BlockSizes) -> tuple[int, int]:
    if block_sizes.b is None:
        raise PallasUnsupportedError(f"Pallas requires non-None b block size, got {block_sizes}.")
    ...
    *,
    ...
    def pad_non_aligned_v_block():
        if v_dim % v_block_size != 0:
            ...
            w_ref[rem:, :] = jnp.zeros((w_ref.shape[0] - rem, w_ref.shape[1]), dtype=w_ref.dtype)
    ...
    *,
    ...
    x_grad_f32, w_grad_partial_f32 = pl.pallas_call(
        ...
        scratch_shapes=(
            ...
            pltpu.VMEM((block_sizes.b, block_sizes.h), dtype=jnp.float32),  # x_grad_tile
            pltpu.VMEM((block_sizes.v, block_sizes.h), dtype=jnp.float32),  # w_grad_tile
