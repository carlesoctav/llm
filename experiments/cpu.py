...
def get_config():
    ...
    config.project_name = ""
    config.exp_name = ""
    config.dir = "/home/carlesoctav/llm2/test"
    config.seed = 42
    ...
    config.tokenizer_id = "google/gemma-3-1b-it"
    ...
    config.model_name = (
        "huggingface.gemma3.Gemma3ForCausalLM.init"
    )
    config.model.config = Gemma3TextConfig(
        num_hidden_layers=1,
        hidden_size=256,
    ...
    def transforms_config():
        return {
            "column": "messages",
            "max_length": 128,
            "tokenizer": lambda: AutoTokenizer.from_pretrained(config.tokenizer_id),
            "data_type": "chat",
