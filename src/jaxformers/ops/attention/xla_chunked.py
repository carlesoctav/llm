...
from tokamax._src import jaxtyping, quantization, shape as shape_lib
        ...
        def q_loop_fn(q_chunk_idx, _):
            ...
            if seq_k >= kv_chunk_size:
                loop_fn = functools.partial(
                    kv_loop_fn, kv_chunk_size_local=kv_chunk_size
                )
            ...
            if (k_remainder := (seq_k % kv_chunk_size)) != 0:
                carry = kv_loop_fn(
                    even_chunks + 1, carry, kv_chunk_size_local=k_remainder
                )
...
class TokamaxRematXlaChunkedDotProductAttention(
