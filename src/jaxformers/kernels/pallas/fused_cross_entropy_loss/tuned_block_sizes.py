...
def infer_block_sizes(
    ...
    *,
    ...
    if device_key == "TPU v4":
        # TPU v4 has a smaller VMEM budget than newer chips; large H/V tiles
        # can trigger Mosaic register spills and compile-time VMEM OOMs for
        # transformer-class shapes (e.g. H=2560, V~150k). Use a more
        # conservative fallback that compiles reliably.
        default_entry = BlockSizes(
            b_block_size=default_entry.b_block_size,
            h_block_size=128,
            v_block_size=128,
        )
